...
- **The repo clone is optional (runs in CoWork or Claude Code).** This skill drafts from the canonical Google Doc master (below) and its bundled references — not from repo files — so it does **not** require a `project-scoping-tool` clone. Do not block on `calibration/`/`templates/` being present.
  - **If** a clone IS the working directory (`calibration/` and `templates/` exist), verify it's current: `git fetch origin`, and if the checked-out branch or `main` is behind `origin/main`, fast-forward `main` (never when `main` is the checked-out branch with uncommitted changes; never discard someone else's in-progress work). Report the finding.
  - **If** there is no clone (CoWork, or any other working directory), skip the git checks and proceed. Write working files (`build_docx.js`, `{Client}_decisions.md`, the draft `.docx`) into the current working directory — a `{Client}/` subfolder if you can create one. The durable outputs don't depend on the repo: the branded docx goes to the Partners Drive `Contracts/` folder (Step 4) and the row goes to the Notion Contracts store (Step 7).
