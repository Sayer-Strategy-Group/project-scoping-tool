...
VAULT_ROOT = Path(os.environ.get("SAYER_VAULT_PATH", "~/Obsidian/SayerBrain")).expanduser()
VAULT_ACCOUNTS = VAULT_ROOT / "03-accounts"
...
def scaffold_folder(
    ...
    *,
    ...
    if dry_run:
        ...
        if vault and VAULT_ACCOUNTS.exists():
            print(f"[dry-run] Would also mirror to vault: {VAULT_ACCOUNTS / folder_name}/")
        elif vault:
            print(
                f"[dry-run] Vault mirror would be skipped — no Obsidian vault at "
                f"{VAULT_ACCOUNTS} (set SAYER_VAULT_PATH to enable)"
            )
        return folder
    ...
    if vault and VAULT_ACCOUNTS.exists():
        vault_folder = VAULT_ACCOUNTS / folder_name
        vault_folder.mkdir(parents=True, exist_ok=True)
        for path, body in files:
            vault_path = vault_folder / path.name
            vault_path.write_text(body, encoding="utf-8")
            print(f"  [vault] wrote {vault_path}")
    elif vault:
        # Obsidian mirror requested but the vault isn't present on this machine
        # (e.g. a teammate with no SayerBrain vault). The repo folder above is
        # the source of truth; skip the mirror with a notice instead of failing.
        print(
            f"  [vault] skipped — no Obsidian vault at {VAULT_ACCOUNTS} "
            "(set SAYER_VAULT_PATH to enable the mirror)"
        )
    return folder
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--no-vault",
        action="store_true",
        help="Skip mirroring files to the Obsidian vault.",
    )
    return p
